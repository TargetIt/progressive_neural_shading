# Phase 6.2 Test Suite — MLP Training
...
def test_network_forward_pass():
    ...
    try:
        ...
        check(not np.any(np.isnan(result)), "no NaN values in output")
        app.window.close()
    except Exception as e: check(False, str(e))
...
def test_backward_compat():
    ...
    try:
        ...
        check(mod is not None, "Phase 6.1 shader OK")
        app.window.close()
    except Exception as e: check(False, str(e))
...
def main():
    global passed, failed
    print("=" * 50)
    print("Phase 6.2 MLP Training - Test Suite")
    print("=" * 50)
    test_shader_compile(); test_network_forward_pass(); test_training_loop(); test_backward_compat()
    total = passed + failed
