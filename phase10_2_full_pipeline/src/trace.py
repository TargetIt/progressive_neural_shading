# Phase 10.2: Trace / Debug Utilities
import numpy as np
...
def pipeline_stats(loss, step, lr, latent, network):
    """Full pipeline progress summary."""
    return {"step": step, "loss": float(np.mean(loss.to_numpy())),
            "lr": lr, "|latent|": float(np.abs(latent.to_numpy()).mean())}
