# Phase 7.1 Test Suite — Deep Network Training
...
def test_network_forward_pass():
    ...
    try:
        ...
        check(np.all(result >= 0), "all values >= 0 (ReLU)")
        app.window.close()
    except Exception as e: check(False, str(e))
...
def test_backward_compat():
    ...
    try:
        ...
        check(mod is not None, "Phase 7.0 shader OK")
        app.window.close()
    except Exception as e: check(False, str(e))
...
def main():
    global passed, failed
    print("=" * 50)
    print("Phase 7.1 Deep Network Training - Test Suite")
    print("=" * 50)
